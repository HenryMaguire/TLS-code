...
def steadystate_coupling_dependence(couplings, T_ph, eps, Gamma, w0, T_EM, Gamma_EM):
    plt.figure()
    N = 20
    G = ket([0])
    E = ket([1])
    ...
    plt.xlabel(r"Phonon Coupling Strength $(cm^{-1})$")
    plt.show()

def steadystate_bias_dependence(biases, T_ph, alpha_ph, Gamma, w0, T_EM, Gamma_EM):
    N = 17
    plt.figure()
    G = ket([0])
    E = ket([1])
...
if __name__ == "__main__":
    ...
    alpha_ph = 500/np.pi #0.05*eps/np.pi# Ind.-Boson frame coupling
    beta = beta_f(T_ph)#1/(0.695*T_ph)
    wc = 53. # Ind.-Boson frame phonon cutoff freq
    shift = 0.5*np.pi*alpha_ph
    w0 = eps#-100 # underdamped SD parameter omega_0
    optical_cutoff = 20000.
    if overdamped:
        Gamma = w0**2/wc

    #biases = list(np.linspace(100, 800, 40))[0:-2] +list(np.linspace(800, 1*8065.5, 10))
    #steadystate_bias_dependence(biases, T_ph, alpha_ph, Gamma, w0, T_EM, Gamma_EM)
